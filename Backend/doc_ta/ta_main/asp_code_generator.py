...
class ASPCodeGenerator():
    ...
    # enough hours
    enough_hour = asp_constraints.basic_constraint(":- not class_has_enough_hours(T), subject(T,_,_).")
    # no 3 conseuquitive lectures
    no_three_consecutive_lectures = asp_constraints.basic_constraint(":- class(_, _, D, S), class (_, _, D, S+1), class (_, _, D, S+2), timeslot(D, S).")
    # if 2 lectures in a day they must follow one another
    two_hour_slot = asp_constraints.basic_constraint(":- class(T,_,D,S), class(T,_,D,S+Y), Y=2..15.")
    # capacity check
    room_capacity = asp_constraints.basic_constraint(":- class(T,R,_,_),room(R,C),subject(T,S,_), C<S.")
    # limit 2 days a week to form 2hour time_slot
    max_two_day_a_week = asp_constraints.basic_constraint(":- not max_two_day_a_week(T), subject(T,_,_).")
    # unique timeslot for each year, allow clashes if stated
    unique_timeslot_unless_allowed = asp_constraints.basic_constraint(":- class_with_year(A,_,D,S,Y), class_with_year(B,_,D,S,Y), A!=B, not clash(A,B).")
    # Students have maximum 6 hours a days
    max_six_hour_a_day = asp_constraints.basic_constraint(":- not max_six_hour_a_day(D), timeslot(D,_).")
    # Each room is used for one lecture each timeslot
    unique_room = asp_constraints.basic_constraint(":- class(T,R1,D,_), class(T,R2,D,_), R1!=R2.")
